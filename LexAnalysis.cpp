...
bool Value::Set(const vector<Value> &a)
{
  type = array_type;
  this->a = a;
  return true;
...
const char *const Lexic_analyzer::KeyWords[] = {"while", "do", "if", "then", "else", "lire", "ecrire", "func", "array"};
...
void Lexic_analyzer::TryProcess (const char *s)
{
  ...
  if (-1 != n)
	...
	AddLex(Numbers, res, num);
      else throw Exception(NUM_ERR);
    }
  
  else if (IsIdentifier(s))
    AddLex(Identifiers, Identifier (s), id);
  
  else throw Exception(IDENT_ERR);
}
...
#define SPACE_TOKENS " \n\t\r"
  ...
  if (s)
	...
	{ 
	  if ( !(comma_end = strstr(comma_start + 1, "\"")) ) throw Exception(COMMA_ERR);
